...
#define HUGO_LOCK   // spin_lock
#define HUGO_UNLOCK // spin_unlock

/* Debug macros*/
#if defined(DEBUG)      /* limited debug messages */
#define GPSDRV_DBG(fmt, arg...)  printk(KERN_INFO "[GPS] (gpsdrv):"fmt"\n" , ## arg)
#define GPSDRV_VER(fmt, arg...)
#define GPSDRV_ERR(fmt, arg...)  printk(KERN_ERR "[GPS] (gpsdrv):"fmt"\n" , ## arg)
#elif defined(VERBOSE)      /* very verbose */
#define GPSDRV_DBG(fmt, arg...)  printk(KERN_INFO "[GPS] (gpsdrv):"fmt"\n" , ## arg)
#define GPSDRV_VER(fmt, arg...)  printk(KERN_INFO "[GPS] (gpsdrv):"fmt"\n" , ## arg)
#define GPSDRV_ERR(fmt, arg...)  printk(KERN_ERR "[GPS] (gpsdrv):"fmt"\n" , ## arg)
...
long gpsdrv_st_recv(void *arg, struct sk_buff *skb)
{
    ...
    printk(KERN_INFO"data start of %d >>\n",skb->len);
    print_hex_dump(KERN_INFO, ">in>", DUMP_PREFIX_NONE,
            16, 1, skb->data, temp, 0);
    printk(KERN_INFO"\n<< end\n");
#endif
    /* Check the Opcode */
             ...
             size_t size, loff_t *offset)
{
    ...
    if (0 != hgps->tx_count) {
        ...
        /* Add it to TX queue */
		spin_unlock(&hgps->lock);
        GPSDRV_VER(" SKB added to Tx queue, Tx count = %x ", hgps->tx_count);
        skb_queue_tail(&hgps->tx_list, skb);
        /* This is added for precaution in the case that there is a context
         * switch during the execution of the above lines. Redundant otherwise.
         */
        if ((0 != hgps->tx_count) && \
            (!skb_queue_empty(&hgps->tx_list))) {
            /* Schedule the Tx-task let */
            GPSDRV_VER(" Scheduling tasklet to write in gpsdrv_write");
            tasklet_schedule(&hgps->gpsdrv_tx_tsklet);
        }
    }
		...
		unsigned int cmd, unsigned long arg)
{
    ...
    hgps = file->private_data;
    if (hgps == NULL) {
                GPSDRV_ERR("Unable to get private_data in function: %s", __func__);
        return -EINVAL;
    }	
